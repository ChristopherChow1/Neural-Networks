...
This is where the deep neural network is implemented
'''
...
def f_fwd(x, w1, w2, w3):  # feed forward
    # hidden layer
    z1 = x.dot(w1)  # dot product of input and weights1
...
def backpropagation(x, y, w1, w2, w3, alpha):
    ...
    z3 = a2.dot(w3)  # from layer 3 to layer 4
    a3 = sigmoid(z3)  # output of layer 4

    # error of output layer
    d3 = (a3 - y)
    d2 = np.multiply((w3.dot((d3.T))).T, (np.multiply(a2, 1 - a2)))
    d1 = np.multiply((w2.dot((d2.T))).T, (np.multiply(a1, 1 - a1)))

    # gradient of w1, w2, and w3
    w1_grad = x.T.dot(d1)
    w2_grad = a1.T.dot(d2)
    w3_grad = a2.T.dot(d3)
    ...
    return w1, w2, w3  # return the new weights
...
def train(x, Y, w1, w2, w3, alpha=0.01, epoch=10):
    ...
    for j in range(epoch):
        ...
        for i in range(len(x)):
            out = f_fwd(x[i], w1, w2, w3)
            l.append((loss(out, Y[i])))
            w1, w2, w3 = backpropagation(x[i], Y[i], w1, w2, w3, alpha)
        lossVal = sum(l) / len(x)
        accuracy = (1 - lossVal) * 100
        print("epochs:", j + 1, "======== accuracy:", accuracy)
        accuracyList.append(accuracy)
        losses.append(loss)
    return accuracyList, losses, w1, w2, w3
...
w2 = init_wt(64, 16)  # also set the first parameter to the same number of neurons
w3 = init_wt(16,2)
acc, losses, w1, w2, w3 = train(xTrainList, yTrainOneHotArray, w1, w2, w3, 0.01, 100)

finalAcc, finalLoss = predict(xTestList, yTestOneHotArray, w1, w2, w3)
